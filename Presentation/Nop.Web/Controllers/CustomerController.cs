    ...
    public partial class CustomerController : BasePublicController
    {
        ...
        public virtual IActionResult Register(RegisterModel model, string returnUrl, bool captchaValid, IFormCollection form)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (registrationResult.Success)
                {
                    ...
                    if (model.UserType == CustomerTypeEnum.Vendor)
                    {
                       // create vendor if vendor selected
        ...
        public virtual IActionResult AccountActivation(string token, string email, Guid guid)
        {
            ...
            var model = new AccountActivationModel
            {
                ...
                CompanyRequired = _customerSettings.CompanyRequired,
                CountryEnabled = _customerSettings.CountryEnabled,
                CountryRequired = _customerSettings.CountryRequired,
                PhoneEnabled = _customerSettings.PhoneEnabled,
                PhoneRequired = _customerSettings.PhoneRequired,
        ...
        //available even when navigation is not allowed
        [CheckAccessPublicStore(true)]
        public virtual IActionResult AccountActivation(AccountActivationModel model, string token, bool captchaValid)
        {

            var customer = _customerService.GetCustomerByEmail(model.Email);
            var cToken = _genericAttributeService.GetAttribute<string>(customer, NopCustomerDefaults.AccountActivationTokenAttribute);
            if (!string.IsNullOrEmpty(token) && token.Equals(cToken))
            {
                if (ModelState.IsValid)
                {
                    //form fields
                    ...
                    _customerRegistrationService.InsertPassword(customer, model.Password, _customerSettings.DefaultPasswordFormat);
            ...
            else
            {
                return
                    View(new AccountActivationModel
                    {
                        Result = _localizationService.GetResource("Account.AccountActivation.InvalideToken")
                    });
            }
        ...
        # region My Account / MyDocumets
        [HttpsRequirement]
        public virtual IActionResult CustomerDocuments()
        {
            if (!_customerService.IsRegistered(_workContext.CurrentCustomer))
                return Challenge();
