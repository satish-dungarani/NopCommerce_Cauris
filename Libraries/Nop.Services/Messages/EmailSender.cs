...
namespace Nop.Services.Messages
{
    ...
    public partial class EmailSender : IEmailSender
    {
            ...
            int attachedDownloadId = 0, IDictionary<string, string> headers = null)
        {
            ...
            if (!string.IsNullOrEmpty(replyTo))
            {
                message.ReplyTo = new MailAddress(replyTo, replyToName);
            }
            ...
            if (!string.IsNullOrEmpty(attachmentFilePath) && _fileProvider.FileExists(attachmentFilePath))
            {
                Attachment data = new Attachment(attachmentFilePath, System.Net.Mime.MediaTypeNames.Application.Octet);
            ...
            //send email
            SmtpClient SmtpServer = new SmtpClient(emailAccount.Host);
            var mail = new MailMessage();
            SmtpServer.EnableSsl = emailAccount.EnableSsl;
            SmtpServer.Port = emailAccount.Port;
            SmtpServer.UseDefaultCredentials = false;
            SmtpServer.Credentials = new System.Net.NetworkCredential(emailAccount.Email, EncryptionHelper.Decrypt(emailAccount.Password));
            ...
            //SmtpServer.Port = emailAccount.Port;
            //SmtpServer.UseDefaultCredentials = false;
            //SmtpServer.Credentials = new System.Net.NetworkCredential(emailAccount.Email, emailAccount.Password);
